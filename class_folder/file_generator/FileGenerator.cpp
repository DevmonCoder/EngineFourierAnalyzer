...
void FileGenerator::saveData(const std::vector<int>& samples,
    ...
    if (file.is_open()) {
        file << std::setw(5) << std::right << "Index" << "\t"
            ...
            << std::setw(18) << std::right << "FFT Magnitude" << "\t"
            << "\n";
        for (size_t i = 0; i < samples.size(); ++i) {
            file << std::setw(10) << samples[i] << "\t"
                << std::setw(15) << std::fixed << std::setprecision(8) << time[i] << "\t"
                << std::setw(15) << std::fixed << std::setprecision(8) << values[i] << "\t"
                << std::setw(15) << std::scientific << std::setprecision(5) << positiveFrequencies[i] << "\t"
                << std::setw(15) << std::scientific << std::setprecision(5) << magnitude[i] << "\n";
        }
        file.close();
